function elm=GeneratePhysicsRegulatedELM(Structure,Phy_lay_A,Phy_lay_B,activeFcn,S)
...
for i = 1:Q
    ...
    for j = 2:r
        tb_list = [];
        for z = 1:N
            ta = X(z, i) * tempmxr(mcounting(z):end);
            if z == 1
                tb = ta;
            else
                tb = [tb; ta];
...
Phy_lay_B_save=Phy_lay_B;
A=zeros(size(Phy_lay_B,1),1);
for i = 1:size(Phy_lay_B,1)
    if(min(Phy_lay_B(i,:))==0)
        A(i,1)=1;
    end
end
...
for i = 1:size(Structure,1)
    InN=size(PhyInput(Phy_lay_B(1,:),Structure(i,1)),2);
    HiddenGate{i}=ones(Structure(i,2),InN);
    for j = 1:size(Phy_lay_B,1)
      HiddenGate{i}(j,:)=PhyInput(Phy_lay_B(j,:),Structure(i,1));
    end
    elm.weight{i}=rand(Structure(i,2),InN).*HiddenGate{i};
    Phy_lay_B=[];
    Phy_lay_B=ones(size(Phy_lay_B_save,1),Structure(i,2));
    for i = 1:size(Phy_lay_B_save,1)
        if A(i,1)==1
            Phy_lay_B(i,:)=0;
            Phy_lay_B(i,i)=1;
        end
