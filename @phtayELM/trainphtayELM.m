function phtayelm = trainphtayELM(phtayelm,Input,Output)
...
for i = 1:1
    ...
    %     end
    % end
    TayLay=PhyInput(Hidden,Structure(1,2));
...
for j = 1:size(phtayelm.weight{2},1)
 ...
 if(phtayelm.bias{1}(j,1)==0)
           answer=(pinv(Hidden(j,:)') * T(:,:)')';
           LW(j,:)=answer;
 else
    LW(j,:)=zeros(1,size(T,1));
 end
end
