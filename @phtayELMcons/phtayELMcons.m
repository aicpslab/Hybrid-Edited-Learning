...
%------------- BEGIN CODE --------------
    ...
    methods
        %% class constructor
        function obj = phtayELMcons(structure,phy_lay,weights,bias,activFcn,cons)
            obj.phy_lay=phy_lay;
        ...
        % 
        output=phtayELMpredictcons(ELM,Input)
        
        % ELM predict

        ELM=trainphtayELMcons(obj,Input,Output)
% ELM predict
...
%         outputIntvl_array = outputSetNARMA(varargin)
    ...
    methods(Static)
phtayELM=GeneratephtayconsELM(structure,phy_lay,weights,bias,activFcn,cons)
