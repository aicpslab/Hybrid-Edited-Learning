...
%------------- BEGIN CODE --------------
    ...
    methods
        %% class constructor
        function obj = phtayHiddenELM(structure,weightgate,weights,bias,activFcn)
            obj.weightgate=weightgate;
            obj.structure = structure;
            obj.weight = weights;
            obj.bias = bias;
            obj.trainingError = [];
            obj.activFcn = activFcn;
%            obj.diva=diva;
        end
        ...
        % 
        output=phtayHiddenELMpredict(ELM,Input)
        
        % ELM predict

        ELM=trainphtayHiddenELM(obj,Input,Output)
% ELM predict
...
%         outputIntvl_array = outputSetNARMA(varargin)
    ...
    methods(Static)
phtayELM=GeneratephtayHiddenELM(structure,Phy_lay,activFcn)
