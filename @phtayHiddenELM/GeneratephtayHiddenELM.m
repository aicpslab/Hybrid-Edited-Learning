function elm=GeneratephtayHiddenELM(Structure,WeightGate,activation)
...
elm.weight{1}=rand(size(PhyInput(ones(Structure(1,1),1),Structure(2,1)),1),Structure(3,1));
elm.bias{1}=zeros(Structure(3,1),1);
...
%     end
% end
% weight{2}=rand(Structure(2,2),Structure(1,2));
...
% Randomly Generate the Input Weight Matrix
    ...
    % end
    %elm.weightgate=WeightGate;
    elm.trainingError = [];
    elm=phtayHiddenELM(Structure,WeightGate,elm.weight,elm.bias,activation);
end
