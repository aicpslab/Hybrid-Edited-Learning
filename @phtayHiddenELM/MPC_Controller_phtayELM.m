...
idx=find(cumsum(explained)>98,1);      
%idx=2;
...
for j = 1 : size(intervals,2) 
     ...
     phtayELM(j)=trainphtayELM(phtayELMs1,input{1},output{1});
...
for k = 1:size(segmentIndex,2)
     if(partitions.ifin(coeff(:,1:idx)'*(xp-mu'),segmentIndex{k},Dimension)==1)
                uB(:,i+1)= phtayELMpredict(phtayELM(k),xp);
                flag=1;
